<div class="pb-16 md:pt-32 ">
    <div class="flex flex-col justify-center items-center h-24">
        <nav>
            <ul class="flex gap-16">
                <li><a href="#" class="nav-element">À propos de nous</a></li>
                <li><a href="#" class="nav-element">Nos Produits</a></li>
                <li><a href="#" class="nav-element">Contact</a></li>
            </ul>
        </nav>
        <div class="w-full flex justify-around md:justify-between items-center h-24 text">
            <div><p>contact@mail.fr</p></div>
            <div><p>+33 724 800 900</p></div>
        </div>
        <div class="flex justify-center items-center">
            <p style="color: #374955">
                <span class="font-medium">Made with </span>
                <i class="fa-regular fa-heart text-red-600"></i>
                <span class="font-medium">by</span>
                <span class="font-extrabold decoration-double underline">Advanced Digital</span>
            </p>
        </div>
    </div>

</div>